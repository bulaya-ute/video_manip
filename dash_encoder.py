...
def encode_and_package(vid_filename, res, out_dir):
    ...
    # Configure logging
    logging.basicConfig(filename=os.path.join(out_dir, 'encoding.log'), level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    for resolution in res:
        try:
            ...
            output_file = os.path.join(out_dir, f"{base_name}_{resolution}.mp4")
    ...
    try:
        dash_manifest = os.path.join(out_dir, f"{base_name}_dash.mpd")
        ffmpeg_cmd = ["ffmpeg"]
...
if __name__ == "__main__":
    video_filename = "forest-of-skiers.mp4"  # Replace with your video filename
    resolutions = ["720p", "480p", "360p"]  # Replace with desired resolutions
    output_dir = "output"  # Replace with desired output directory

    if os.path.isfile(video_filename):
        encode_and_package(video_filename, resolutions, output_dir)
    else:
